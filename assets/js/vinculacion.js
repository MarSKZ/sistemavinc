...
//cambio de estado
//funcion para validar el cambio de estado de manera individual
function cambiarEstado(id, nuevoEstado) {
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            window.location.reload(); // Recarga la página después de cambiar el estado
        }
    };
    xhttp.open("GET", "config/cambiarEstado.php?id=" + id + "&estado=" + nuevoEstado, true);
    xhttp.send();
}
//funcion para activar a todos
function activarTodos() {
    var confirmacion = confirm("¿Desea activar a todos los alumnos?");
    if (confirmacion) {
        var estadoNuevo = 1; // Establece el nuevo estado (1 para activar)

        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                window.location.reload(); // Recarga la página después de cambiar el estado de todos los alumnos
            }
        };
        xhttp.open("GET", "proceso/activarDesactivar_todos.php?estado=" + estadoNuevo, true);
        xhttp.send();
    }
}

//funcion para desactivar a todos
function desactivarTodos() {
    var confirmacion = confirm("¿Desea desactivar a todos los alumnos?");
    if (confirmacion) {
        var estadoNuevo = 0; // Establece el nuevo estado (0 para desactivar)
